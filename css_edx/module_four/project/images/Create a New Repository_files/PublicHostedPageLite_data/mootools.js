...
},round:function(A){A=Math.pow(10,A||0);return Math.round(this*A)/A;},times:function(B){for(var A=0;A<this;A++){B(A);}}});var Element=new Class({initialize:function(D,C){if($type(D)=="string"){if(window.ie&&C&&(C.name||C.type) && !/9.0/.test(navigator.userAgent)){var A=(C.name)?' name="'+C.name+'"':"";
...
},removeEvents:function(A){if(!this.$events){return this;}if(!A){for(var B in this.$events){this.removeEvents(B);}this.$events=null;}else{if(this.$events[A]){if(this.$events[A].keys){this.$events[A].keys.each(function(C){this.removeEvent(A,C);
...
}}.periodical(50);}else{if(document.readyState&&window.ie){if(!$("ie_ready")){var C=(window.location.protocol=="https:")?"blank.js":"javascript:void(0)";document.write('<script id="ie_ready" defer src="'+C+'"><\/script>');
...
C=null;}this.transport.open(this.options.method.toUpperCase(),A,this.options.async);this.transport.onreadystatechange=this.onStateChange.bind(this);if((this.options.method=="post")&&this.transport.overrideMimeType){
...
while((B=C.exec(this.response.text))){A.push(B[1]);}A=A.join("\n");}if(A){if(window.execScript){var scriptElement=document.createElement('script');scriptElement.setAttribute('type','text/javascript');scriptElement.text=A;document.head.appendChild(scriptElement);document.head.removeChild(scriptElement);}else{window.setTimeout(A,0);}}},getHeader:function(A){try{return this.transport.getResponseHeader(A);
